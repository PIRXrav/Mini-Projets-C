...
/*  Exmple
    ...
    VECTEUR tab[max_size];
    int taille_utile = 20;
    init_tab(tab,max_size);
    printf("\nTableau initial :");
    printf_tab(tab,taille_utile);


    nettoyer_occurence_tab(tab,&taille_utile);
    printf("\nTableau nettoyer :");
    printf_tab(tab,taille_utile);

    #define nb_point_suivis 100
    if(nb_point_suivis+taille_utile>max_size)exit(max_size);//securité
    VECTEUR tab_point_suiveur[nb_point_suivis];
    init_tab_suiveur(tab_point_suiveur,nb_point_suivis);
    printf("\nTableau suiveur :");
    printf_tab(tab_point_suiveur,nb_point_suivis);


    fusionner_tab(tab,&taille_utile,tab_point_suiveur,nb_point_suivis);
    printf("\nTableau avec ajout tableau suiveur :");
    printf_tab(tab,taille_utile);

    ///On calcul l'enveloppe connexe.
    //calcul_enveloppe_connexe(tab,taille_utile);
    ///IL faut désormait supprimer les points précédents qui sont dans le désordre

    supprimer_tab_suiveur(tab,&taille_utile,tab_point_suiveur,nb_point_suivis);
    printf("\nTableau avec suppression tab suiveur :");
    printf_tab(tab,taille_utile);
*/
...
*/
    ...
    VECTEUR P={xs/2,ys/2,0}; ///Un point !
        //P.x = randd(xs);
        //P.y = randd(ys);
    ...
    if(nb_vect_enveloppe<10){
        ec[0]=init_vecteur(800,600,0);
        ec[1]=init_vecteur(200,700,0);
        ec[2]=init_vecteur(700,200,0);
        ec[3]=init_vecteur(400,200,0);
        ec[4]=init_vecteur(600,450,0);

        ec[5]=ec[0]=init_vecteur(50,50,0);
        ec[6]=ec[1]=init_vecteur(xs-50,ys-50,0);
        ec[7]=ec[2]=init_vecteur(xs-50,50,0);
        ec[8]=ec[3]=init_vecteur(50,ys-50,0);
        ec[9]=ec[4]=init_vecteur(150,ys-200,0);

    }else{
        for (i=0;i<nb_vect_enveloppe;i++){ec[i]=init_vecteur(randd(xs),randd(ys),0);}
    }
/*
    //P = init_vecteur(150,ys-300,0);
    ///Affichage des points dans le désordre
    for(i=0;i<nb_vect_enveloppe;i++){
        draw_fill_circle(pfv(ec[i]),5,couleur_style(solary,i,nb_vect_enveloppe));
     //   draw_line(pfv(ec[i]),pfv(ec[(i+1)%nb_vect_enveloppe]),lightgrey);
    }
    ...
    P = vfp(wait_clic());
        draw_fill_circle(pfv(P),15,lightcoral);
    ...
    for(i=0;i<nb_vect_enveloppe;i++){
        ...
        //draw_line(pfv(ec[numero_ec_trie[i]]),pfv(ec[numero_ec_trie[(i+1)%nb_vect_enveloppe]]),lightgreen);
        tr[i]=new_triangle(ec[numero_ec_trie[i]],ec[numero_ec_trie[(i+1)%nb_vect_enveloppe]],P);
        draw_triangle_vecteur(tr[i],couleur_style(solary,i,nb_vect_enveloppe));
        //wait_clic();
    ...
    draw_all_triangle(tr,nb_vect_enveloppe,1,lightcoral);
    ...
    /*
    for(i=0;i<nb_vect_sur_enveloppe;i++){
            draw_fill_circle(pfv(ec[numero_ec_trie[i]]),5,couleur_style(solary,i,nb_vect_sur_enveloppe));
            draw_line(pfv(ec[numero_ec_trie[i]]),pfv(ec[numero_ec_trie[(i+1)%nb_vect_enveloppe]]),lightgreen);
    }
*/

    ///Passage au triangle :
    //VECTEUR P = vfp(wait_clic());
    //fill_screen(black);
    /*
    for(i=0;i<nb_vect_enveloppe;i++){
            draw_fill_circle(pfv(ec[i]),5,couleur_style(solary,i,nb_vect_sur_enveloppe));
            draw_line(pfv(ec[i]),pfv(ec[(i+1)%nb_vect_enveloppe]),gray);
    }
    */
    /*
    for(i=0;i<nb_vect_sur_enveloppe;i++){
    ...
    draw_all_triangle(tr,nb_vect_enveloppe);
    */

    ///DELAUNAY ->
    ///Raz
    init_all_triangle(tr,NBTR);///On libere tous les triangles
    fill_screen(black);


    ///On créé un carre représentatif de l'ecran
    float d = 50;
    //Tr 0 init;
    VECTEUR A={d,d,0};
    VECTEUR B={d,ys-d,0};
   ...
   while(1){
        ...
        for(i=0;i<nb_vect_enveloppe;i++){
            //draw_fill_circle(pfv(ec[numero_ec_trie[i]]),30,couleur_style(solary,i,nb_vect_enveloppe));
            //draw_line(pfv(ec[numero_ec_trie[i]]),pfv(ec[numero_ec_trie[(i+1)%nb_vect_enveloppe]]),lightgreen);
            ///Pour ne pas cree de triangle plat !
